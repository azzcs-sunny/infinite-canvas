...
const defaultChannelModels = {
    image: DEFAULT_IMAGE_MODELS,
...
function withChannels(config: AiConfig, channels: ModelChannel[]): AiConfig {
    const normalizedChannels = channels.map((channel, index) => (index === 0 ? { ...channel, baseUrl: WCAPIS_BASE_URL, apiFormat: "openai" as const } : channel));
    const next: AiConfig = {
        ...config,
